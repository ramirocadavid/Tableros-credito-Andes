# Ruta PC oficina
setwd("C:/Users/Ramiro/rcadavid@grameenfoundation.org/7. Proyectos/Activos/DelosAndes Cooperativa/Diseño de herramientas/03. ARET/Conexión DBF a Salesforce/Datos")
# Ruta PC personal
setwd("C:/Users/Ramiro Cadavid/rcadavid@grameenfoundation.org/7. Proyectos/Activos/DelosAndes Cooperativa/Dise�o de herramientas/03. ARET/Conexi�n DBF a Salesforce/Datos")
...
sum.vrPagos <- aggregate(x = select(vr.pagos, matches("CREDITO")),
                         by = select(vr.pagos, matches("id.car_pagos")),
                         FUN = sum)
names(sum.vrPagos) <- c("id.join", "vr.pagos")

## Prueba (comentar)
vr.pagos$CREDITO <-format(vr.pagos$CREDITO, scientific = FALSE)
write.csv(vr.pagos, "completo.csv")
...
vrActualAfecta <- ifelse(f.RelCartAsoc$NOAFECTACU == 1,
                         f.RelCartAsoc$vr.actual, 0)
f.RelCartAsoc <- data.frame(f.RelCartAsoc, vrActualAfecta)

vrActualAfecta <- aggregate(x = f.RelCartAsoc$vrActualAfecta,
                            by = select(f.RelCartAsoc, matches("CEDULA")),
                            FUN = sum)
# Prueba (revisar también el aggregate anterior)
write.csv(f.RelCartAsoc, "relcartasoc.csv")
write.csv(vrActualAfecta, "vrActualAfecta.csv")
